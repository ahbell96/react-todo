...
export default class TasksList extends Component {
  componentDidMount() {
    ...
    Database.collection("tasks")
      .get()
      .then((querySnapshot) => {
        const documentsData = querySnapshot.docs.map((documentRep) => ({
          ...documentRep.data(),
          ...{ id: documentRep.id },
        }));
        ...
        {this.props.tasks.map((item, index) => {
          return (
            <TaskItem
              ...
              key={index}
              index={index}
              /*toggleDone={()=>updateTaskAtIndex({...item, ...{done:!item.done}}, index)}*/ done={
                item.done
              }
              updateTaskAtIndex={this.props.updateTaskAtIndex}
